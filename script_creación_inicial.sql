...
BEGIN
	BEGIN TRY
		BEGIN TRANSACTION
			...
			/*
			SELECT ORDEN_TRABAJO_FECHA, CAMION_PATENTE, 
				MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD 
			FROM gd_esquema.Maestra
			WHERE TAREA_CODIGO IS NOT NULL AND MATERIAL_COD IS NOT NULL
			*/
			/*
			INSERT INTO LOS_GEDEDES.material_tarea (codTarea, codMaterial, cantidad)
			SELECT DISTINCT ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO,TAREA_CODIGO, 
			MATERIAL_COD, COUNT(MATERIAL_COD) CANT
			FROM gd_esquema.Maestra
			WHERE TAREA_CODIGO IS NOT NULL AND MATERIAL_COD IS NOT NULL
			GROUP BY ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			ORDER BY TAREA_CODIGO, MATERIAL_COD, ORDEN_TRABAJO_FECHA
			*/
			...
			WHERE TAREA_CODIGO IS NOT NULL AND MATERIAL_COD IS NOT NULL
			GROUP BY ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			HAVING MATERIAL_COD = '987b3' AND TAREA_CODIGO = 1
			ORDER BY TAREA_CODIGO, MATERIAL_COD, ORDEN_TRABAJO_FECHA

			SELECT *
			FROM gd_esquema.Maestra
			WHERE MATERIAL_COD = '987b3' AND TAREA_CODIGO = 1 AND MECANICO_NRO_LEGAJO IN(13487286, 81696591) 
				AND (CAMION_PATENTE ='KR69OD3IR363TWEDN528RZ' OR CAMION_PATENTE = 'Q6WTSOV819D0I3AR7557TL') 
			ORDER BY TAREA_CODIGO, MATERIAL_COD, ORDEN_TRABAJO_FECHA
			
			SELECT DISTINCT TAREA_CODIGO, MATERIAL_COD, COUNT(MATERIAL_COD) CANT
			FROM gd_esquema.Maestra
			WHERE TAREA_CODIGO IS NOT NULL AND MATERIAL_COD IS NOT NULL
			GROUP BY ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			HAVING MATERIAL_COD = '987b3' AND TAREA_CODIGO = 1
			...
			SELECT DISTINCT ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD, COUNT(MATERIAL_COD) CANT
			FROM LOS_GEDEDES.v_Maestra
			GROUP BY ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			ORDER BY TAREA_CODIGO, MATERIAL_COD
			...
			SELECT ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			FROM gd_esquema.Maestra
			WHERE ORDEN_TRABAJO_FECHA = 2019-07-29, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD
			...
			SELECT *
			FROM gd_esquema.Maestra
			WHERE ORDEN_TRABAJO_FECHA = '2019-01-16' AND 
				CAMION_PATENTE = 'KR69OD3IR363TWEDN528RZ' AND 
				MECANICO_NRO_LEGAJO = 13487286 AND MATERIAL_COD = '1ba26' AND
				TAREA_CODIGO = 1
			
			SELECT DISTINCT ORDEN_TRABAJO_ESTADO FROM gd_esquema.Maestra

			SELECT COUNT(DISTINCT MATERIAL_COD) FROM gd_esquema.Maestra

			SELECT TAREA_CODIGO, MATERIAL_COD
			FROM gd_esquema.Maestra
			WHERE TAREA_CODIGO IS NOT NULL AND MATERIAL_COD IS NOT NULL
			ORDER BY TAREA_CODIGO, MATERIAL_COD
			*/
...
BEGIN
	BEGIN TRY
		BEGIN TRANSACTION
			...
			--SELECT DISTINCT MECANICO_NRO_LEGAJO, TALLER_NOMBRE FROM gd_esquema.Maestra
			--WHERE MECANICO_NRO_LEGAJO IS NOT NULL

			--SELECT COUNT(DISTINCT TALLER_NOMBRE) FROM gd_esquema.Maestra
			--WHERE TALLER_NOMBRE IS NOT NULL
...
BEGIN
	BEGIN TRY
		BEGIN TRANSACTION
			...
			--SELECT * FROM LOS_GEDEDES.tarea_orden
			
			--SELECT DISTINCT ORDEN_TRABAJO_FECHA, CAMION_PATENTE, MECANICO_NRO_LEGAJO, TAREA_CODIGO, MATERIAL_COD 
			--FROM gd_esquema.Maestra
			--WHERE ORDEN_TRABAJO_FECHA IS NOT NULL AND MECANICO_NRO_LEGAJO IS NOT NULL
